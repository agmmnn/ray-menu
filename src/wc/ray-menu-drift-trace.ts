 ...
 */
export function updateDriftTrace(
  svg: SVGSVGElement,
  options: DriftTraceOptions,
): void {
  const {
    menuPosition,
    pointerPosition,
  ...
  if (
    showAnchorLine &&
    hoveredAngle !== undefined &&
    distFromCenter > currentRadius
  ) {
    const anchorPoint = {
  ...
  if (showTrailPath) {
    if (trailPoints.length >= 2) {
      const pathData =
        "M " + trailPoints.map((p) => `${p.x},${p.y}`).join(" L ");
    ...
    if (speed > 50) {
      ...
      dirLine.setAttribute(
        "x2",
        String(pointerPosition.x + (velocity.vx / speed) * 30),
      );
      dirLine.setAttribute(
        "y2",
