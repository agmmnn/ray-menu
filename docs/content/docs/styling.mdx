...
- `"menu"` — frost only the active menu level; previous menu traces remain unfrosted (default when attribute has no value)
- `"path"` — frost the active menu plus previously selected segments when diving into submenus
- `"all"` — frost every segment at every level, regardless of selection state

```html
<ray-menu frosted></ray-menu>
<ray-menu frosted="path"></ray-menu>
<ray-menu frosted="all"></ray-menu>
```

Customize the blur intensity:
