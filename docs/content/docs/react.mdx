...
| `frosted`            | `false \| 'menu' \| 'path' \| 'all'`    | `false`   | Frosted glass backdrop-blur on segments  |
...
const menu = useRayMenu({
  ...
  frosted: "path", // frost active menu + selected parent segments
  onSelect: (item) => console.log(item.label),
});
```

The `frosted` prop accepts three modes:

- `"menu"` — frost only the active menu level
- `"path"` — frost the active menu plus previously selected segments in submenus
- `"all"` — frost every segment at every level
